...
class ParticleManagerGUI(QMainWindow):
    def __init__(self, tf_directory: Path):
        ...
        self.addon_manager = AddonsManager()
        ...
        if not self.settings.profiles:
            self.settings.create_profile(Sourcemods.DEFAULT.name, self.initial_tf_directory, Sourcemods.DEFAULT, activate = True)
        else:
    ...
    def rebuild_profile_menu(self):
        ...
        for profile in self.settings.profiles.values():
            ...
            action.triggered.connect(lambda checked, profile_name=profile.name: self.switch_profile(profile_name))
    ...
    def switch_profile(self, profile_name: str):
        active = self.settings.active_profile
        if active and active is self.settings.profiles[profile_name]:
            return

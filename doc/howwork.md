...
### Game_info.txt
Basically, changing one line in tf/game_info.txt disables large portions of the multiplayer sv_pure check, but not all, mind you. Off the top of my head, I believe it allows for custom materials, custom models (not map props), custom animations, as well as a few VGUI elements and lightwarps. Pretty good for something so easy! As previously mentioned, this exploit has been around for some time. I remember reading a reddit thread back in ~2021 talking about its use in CS:GO, referencing the [CS:GO bugs repository](https://github.com/kkthxbye-code/csgo_bugs/tree/master). As I hadn't played tf2 in over a year at that point, I didn't think much about it, or even attempt to make a connection.
